...
Usage with external JSON data:

```tsx
const chapters = useAsyncAsset(
  () => fetch(staticFile('data/chapters.json')).then(r => r.json()),
  []
);
```
...
```tsx
import { preloadFont } from 'remotion';

const usePreloadFont = (fontFamily: string, url: string) => {
...
// In component:
usePreloadFont('Oswald', 'https://fonts.gstatic.com/s/oswald/v53/TK3iWkUHHAIjg752FD8Ghe4.woff2');
```
...
### Composing Scenes with TransitionSeries

For crossfade/slide transitions between scenes (better than hard cuts):
...
For webhook-driven video (CRM data → composition props):
...
export const personalizedSchema = z.object({
  ...
  // NEVER put large objects in defaultProps
  // Pass IDs, load data inside component with staticFile() or fetch()
});
```
